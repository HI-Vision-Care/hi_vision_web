...
} from "@/services/consultant/consultant";
...
export default function ChatInterface({
  ...
  useEffect(() => {
    ...
    getOnGoingConsultations(staffAccountId)
      ...
      .catch((err) => {
        console.error("Error getting list of customers texting:", err);
      });
  }, [staffAccountId]);
  ...
  useEffect(() => {
    if (
      selectedPatient &&
      !chatUsers.find((u) => u.accountID === selectedPatient.accountID)
    ) {
      const newChatUser: ChatUser = {
        ...
        isOnline: true,
        note: selectedPatient.note || "Customers need advice",
        lastMessageTime: "now",
        unreadCount: 1,
      };
  ...
  const sendMessage = () => {
    if (!currentMessage.trim() || !stompClientRef.current || !selectedPatient)
      return;
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {selectedPatient.isOnline ? "Active now" : "Offline"}
                  </p>
                </div>
              </div>
                    ...
                    try {
                      ...
                      setChatUsers((prev) => {
                        const newList = prev.filter(
                          (user) => user.accountID !== selectedPatient.accountID
                        );
